// Extracted by i18n codemod from app/forgot-password/page.tsx.
export default {
  passwordResetIsnTConfiguredFor: { ms: "Set semula kata laluan belum dikonfigurasi untuk deployment ini.", en: "Password reset isn't configured for this deployment yet." },
  checkYourEmail: { ms: "SEMAK EMEL ANDA", en: "CHECK YOUR EMAIL" },
  aPasswordResetLinkIsOn: { ms: "Pautan set semula kata laluan telah dihantar.", en: "A password reset link is on its way." },
  ifIsRegisteredAPasswordReset: { ms: "Jika {email} berdaftar, pautan set semula kata laluan telah dihantar. Sahkan emel anda untuk teruskan.", en: "If {email} is registered, a password reset link has been sent. Check your inbox to continue." },
  goToLogin: { ms: "KE LOG MASUK »", en: "GO TO LOGIN »" },
  resetPassword: { ms: "SET SEMULA KATA LALUAN", en: "RESET PASSWORD" },
  enterYourEmailToReceiveA: { ms: "Masukkan emel untuk terima pautan set semula.", en: "Enter your email to receive a reset link." },
  email: { ms: "EMEL", en: "EMAIL" },
  sending: { ms: "MENGHANTAR…", en: "SENDING…" },
  sendLink: { ms: "HANTAR PAUTAN »", en: "SEND LINK »" },
  backToLogin: { ms: "Kembali ke log masuk", en: "Back to login" },
};
