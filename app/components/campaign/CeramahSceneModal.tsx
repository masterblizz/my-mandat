...
export default function CeramahSceneModal({ stateId, gameType, tactic, onClose }: CeramahSceneModalProps) {
  ...
  const prnCandidate = settings.electionScope === "prn" ? getPrnCandidate(journey.prnCandidateId) : undefined;
  ...
  const projectedGain = useMemo(() => {
    ...
    return Math.round((calculateCampaignGain(targetState, gameType, tactic) + bonus + manifestoGain + candidateGain) * 100) / 100;
  }, [targetState, gameType, tactic, settings.electionScope, stateId, selectedTopic, careerProgress.term, journey.prnIssueActions, manifestoGain, candidateGain]);
          ...
          <div className="w-full mt-1">
            {phase === "topic" && (
              <>
                ...
                {prnCandidate && <div className="mb-3 border px-3 py-2 text-center text-[10px]" style={{ borderColor: `${prnCandidate.color}66`, color: prnCandidate.color }}>
                  {prnCandidate.name} · {prnCandidate.archetype[lang]} · {gameType === prnCandidate.preferredChannel ? t(lang, "saluran calon utama", "candidate's preferred channel") : t(lang, "saluran calon lemah", "candidate's weaker channel")} · {candidateGain >= 0 ? "+" : ""}{candidateGain.toFixed(2)}
                </div>}
