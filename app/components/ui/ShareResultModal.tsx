...
function drawResultCard(canvas: HTMLCanvasElement, data: ShareResultData, lang: ReturnType<typeof useLang>) {
  ...
  ctx.font = "600 14px 'Space Mono', monospace";
  ctx.fillStyle = PALETTE.text;
  ctx.fillText(`${t(lang, "components_ui_ShareResultModal.seats")} ${data.mandatSeats}/${data.totalSeats}`, barX, barY + barH + 30);
  ctx.textAlign = "right";
  ctx.fillStyle = PALETTE.muted;
  ctx.fillText(t(lang, "components_ui_ShareResultModal.majorityTarget", { dataMajorityTarget: data.majorityTarget }), barX + barW, barY + barH + 30);
  ctx.textAlign = "left";

  // Stat tiles
  const tiles = [
    { label: t(lang, "components_ui_ShareResultModal.nationalSupport"), value: `${data.nationalSupport}%` },
    { label: t(lang, "components_ui_ShareResultModal.campaignDays"), value: `${data.day}/${data.totalDays}` },
    { label: t(lang, "components_ui_ShareResultModal.achievements"), value: `${data.achievedCount}/${data.totalAchievements}` },
  ];
  const tileY = 330;
  const tileH = 108;
  const gap = 20;
  ...
  ctx.fillText(t(lang, "components_ui_ShareResultModal.generatedByMyMandatAMalaysian"), barX, CARD_H - 44);
}
...
export default function ShareResultModal({ data, onClose }: ShareResultModalProps) {
  ...
  function handleDownload() {
    ...
    canvas.toBlob((blob) => {
      ...
      setStatus(t(lang, "components_ui_ShareResultModal.imageDownloaded"));
    }, "image/png");
  }
  ...
  async function handleCopy() {
    ...
    if (!navigator.clipboard || !("ClipboardItem" in window)) {
      setStatus(t(lang, "components_ui_ShareResultModal.copyNotSupportedDownloadOnly"));
      return;
    }
    canvas.toBlob(async (blob) => {
      ...
      try {
        ...
        setStatus(t(lang, "components_ui_ShareResultModal.imageCopiedPasteAnywhere"));
      } catch {
        setStatus(t(lang, "components_ui_ShareResultModal.copyFailed"));
      }
    }, "image/png");
  }
      ...
      >
        <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: "1px solid rgb(var(--cyan-rgb) / 0.2)" }}>
          <span className="text-[14px] font-bold tracking-widest uppercase" style={{ color: "var(--gold)" }}>
            {t(lang, "components_ui_ShareResultModal.shareResult")}
          </span>
          <button onClick={onClose} className="text-[22px] leading-none" style={{ color: "var(--text-muted)", cursor: "pointer" }} aria-label={t(lang, "components_ui_ShareResultModal.close")}>
            ×
          </button>
        </div>
        ...
        <div className="px-5 py-5 flex flex-col items-center gap-4">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3">
            ...
            >
              {t(lang, "components_ui_ShareResultModal.downloadImage")}
            </button>
            <button
              onClick={handleCopy}
              className="px-5 py-2.5 text-[12px] font-bold tracking-widest uppercase"
            ...
            >
              {t(lang, "components_ui_ShareResultModal.copyImage")}
            </button>
          </div>
          {status && (
