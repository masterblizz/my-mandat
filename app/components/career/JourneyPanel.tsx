...
import { CAREER_STORIES } from "../../data/careerStories";
...
export default function JourneyPanel({ local = false }: { local?: boolean }) {
  ...
  const story = CAREER_STORIES[(campaign ? s.day - 1 : Math.floor((s.careerProgress.month - 1) / 3)) % CAREER_STORIES.length];
  const storyAvailable = !j.storyResolved.includes(storyKey) && (activeCampaign || term);
  ...
  return <section aria-label={t(lang, "Taklimat kerjaya", "Career briefing")} className="mb-5 border border-cyan/30 bg-black/20 p-4 text-sm">
    ...
    {storyAvailable && <div className="mt-4 border border-gold/30 p-3">
      ...
      <div className="flex flex-wrap gap-2">{([ ["help", "Sokong · RM50,000 · hubungan +8, kepercayaan −2", "Back them · RM50,000 · relationship +8, trust −2", 50000], ["compromise", "Acara kecil · RM20,000 · hubungan +3, kepercayaan +1", "Smaller event · RM20,000 · relationship +3, trust +1", 20000], ["refuse", "Utamakan rakyat · hubungan −8, kepercayaan +2", "Prioritise residents · relationship −8, trust +2", 0] ] as const).map(([choice, ms, en, cost]) => <button key={choice} className={button} disabled={!available || s.resources.funds < cost} onClick={() => s.journeyAction({ type: "story", choice, character: characterName })}>{story.choices[choice][lang]} · {t(lang, ms, en).split(" · ").slice(1).join(" · ")}</button>)}</div>
    </div>}
