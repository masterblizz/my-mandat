...
export default function PoliticalOfficePage() {
  ...
  const lang = useLang();
  const { leader, journey, states, day, totalDays, advanceDay, runLocationActivity, markOfficeMailRead } = useGameStore();
  ...
  const hotspots: Hotspot[] = [
    { icon: "🖥️", title: t(lang, "Meja Strategi", "Strategy Desk"), detail: t(lang, "Rancang kempen, agih dana dan tetapkan mesej utama untuk minggu ini.", "Plan campaign moves, allocate funds and set the week's core message."), className: "left-[36%] top-[51%]" },
    { icon: "📅", title: t(lang, "Jadual Politik", "Political Schedule"), detail: t(lang, "Rancang hari kempen dan lawatan tanpa meninggalkan pejabat.", "Plan campaign days and visits without leaving the office."), className: "left-[52%] top-[25%]" },
    { icon: "📰", title: t(lang, "Berita TV", "Live News"), detail: t(lang, "Semak mesej dan respons awam daripada bilik kawalan ini.", "Review messaging and public response from this control room."), className: "left-[70%] top-[16%]" },
    { icon: "🗂️", title: t(lang, "Fail Kawasan", "Constituency Files"), detail: t(lang, "Semak isu zon dan rekod penduduk di dalam pejabat.", "Review zone issues and resident records inside the office."), className: "left-[12%] top-[32%]" },
  ];
  const mails = lang === "ms"
    ? [
  ...
  return <div className="min-h-screen overflow-hidden" style={{ background: "#020814" }}>
    ...
    <main className="relative h-[calc(100vh-30px)] min-h-[650px] pt-[40px]" style={{ fontFamily: "'Space Mono', monospace" }}>
      ...
      {notice && <div role="status" className="absolute left-1/2 top-14 z-30 -translate-x-1/2 border px-4 py-2 text-[9px] font-black tracking-widest text-gold shadow-2xl" style={{ borderColor: "rgb(var(--gold-rgb) / .55)", background: "rgb(2 8 20 / .94)" }}>{notice}</div>}
      ...
      {assistantOpen && <section className="absolute bottom-12 left-4 z-20 flex w-[min(500px,calc(100%-32px))] items-end gap-3" ><button type="button" onClick={() => setAssistantOpen(false)} className="relative h-28 w-28 shrink-0 overflow-hidden rounded-full border-2 shadow-2xl" style={{ borderColor: "var(--gold)", boxShadow: "0 0 24px rgb(var(--gold-rgb) / .42)" }}><Image src="/personal-assistant.png" alt="Personal Assistant" fill sizes="112px" className="object-cover" /></button><div className="flex-1 border p-3 shadow-2xl" style={{ borderColor: "rgb(var(--gold-rgb) / .54)", background: "rgb(2 8 20 / .94)", backdropFilter: "blur(14px)" }}><div className="flex items-start justify-between gap-3"><div><div className="text-[9px] font-black tracking-[.2em] text-gold">PEMBANTU PERIBADI</div><h2 className="mt-1 text-[13px] font-black text-white">{objectiveDone ? t(lang, "Objektif pertama berjaya dicapai.", "Your first objective is complete.") : t(lang, "Naikkan sokongan kawasan ke 60% sebelum hari ke-10.", "Raise constituency support to 60% before day 10.")}</h2></div><button type="button" onClick={() => setAssistantOpen(false)} className="text-text-muted">×</button></div><div className="mt-2 border px-2 py-1.5 text-[9px]" style={{ borderColor: "rgb(var(--cyan-rgb) / .35)", background: "rgb(var(--cyan-rgb) / .06)" }}><b className="text-cyan">{homeSupport.toFixed(1)}% / 60%</b><span className="ml-2 text-text-muted">{t(lang, `Hari ${day}/${objectiveDay} · ganjaran RM75,000`, `Day ${day}/${objectiveDay} · RM75,000 reward`)}</span></div><p className="mt-2 text-[10px] leading-relaxed text-text-muted">{t(lang, "Gunakan jadual, media atau meja strategi. Setiap tindakan kini memberi kesan kepada objektif ini.", "Use the schedule, media or strategy desk. Every action now contributes to this objective.")}</p><div className="mt-3 flex gap-2"><button type="button" onClick={() => setActive(hotspots[1].title)} className="flex-1 border px-2 py-2 text-[9px] font-black text-cyan" style={{ borderColor: "rgb(var(--cyan-rgb) / .55)" }}>{t(lang, "ATUR LAWATAN", "SCHEDULE VISIT")}</button><button type="button" onClick={() => setActive(hotspots[2].title)} className="flex-1 border px-2 py-2 text-[9px] font-black text-cyan" style={{ borderColor: "rgb(var(--cyan-rgb) / .55)" }}>{t(lang, "KENYATAAN MEDIA", "MEDIA STATEMENT")}</button></div></div></section>}
      {!assistantOpen && <button type="button" onClick={() => setAssistantOpen(true)} className="absolute bottom-12 left-4 z-20 h-16 w-16 overflow-hidden rounded-full border-2" style={{ borderColor: "var(--gold)" }}><Image src="/personal-assistant.png" alt="Personal Assistant" fill sizes="64px" className="object-cover" /></button>}

      {active && (() => { const spot = hotspots.find((item) => item.title === active)!; return <section className="absolute bottom-12 left-1/2 z-30 w-[min(390px,calc(100%-32px))] -translate-x-1/2 border p-4 shadow-2xl" style={{ borderColor: "rgb(var(--cyan-rgb) / .6)", background: "rgb(2 8 20 / .96)", backdropFilter: "blur(16px)" }}><div className="flex items-start justify-between gap-3"><div><div className="text-[9px] font-black tracking-widest text-gold">{spot.icon} {spot.title.toUpperCase()}</div><p className="mt-2 text-[11px] leading-relaxed text-text-muted">{spot.detail}</p><p className="mt-2 text-[9px] text-cyan">{t(lang, "Setiap pilihan menggunakan 1 tenaga dan direkod dalam jurnal karier.", "Each choice uses 1 energy and is recorded in the career journal.")}</p></div><button type="button" onClick={() => setActive(null)} className="text-text-muted">×</button></div><div className="mt-3 grid grid-cols-2 gap-2"><button type="button" onClick={() => runOfficeActivity("prepare", spot.title)} className="border px-3 py-2 text-[9px] font-black tracking-widest text-cyan" style={{ borderColor: "rgb(var(--cyan-rgb) / .55)", background: "rgb(var(--cyan-rgb) / .08)" }}>{t(lang, "SEDIAKAN", "PREPARE")}</button><button type="button" onClick={() => runOfficeActivity("commit", spot.title)} className="border px-3 py-2 text-[9px] font-black tracking-widest text-gold" style={{ borderColor: "rgb(var(--gold-rgb) / .55)" }}>{t(lang, "LAKSANAKAN", "EXECUTE")}</button></div></section>; })()}
      {inboxOpen && <section className="absolute right-4 top-40 z-30 w-[min(320px,calc(100%-32px))] border border-t-2 shadow-2xl" style={{ borderColor: "rgb(var(--cyan-rgb) / .42)", borderTopColor: "var(--neon-red)", background: "rgb(2 8 20 / .96)", backdropFilter: "blur(16px)" }}><div className="flex items-center justify-between border-b px-4 py-3" style={{ borderColor: "rgb(var(--cyan-rgb) / .22)" }}><div><div className="text-[9px] font-black tracking-[.2em] text-cyan">✉️ {t(lang, "E-MEL MASUK", "INBOX")}</div><h2 className="mt-1 text-sm font-black text-white">{t(lang, "Peti masuk pejabat", "Office inbox")}</h2></div><div className="flex gap-2"><b className="bg-neon-red px-1.5 py-1 text-[8px] text-white">{mails.length - journey.readOfficeMail.length} {t(lang, "BARU", "NEW")}</b><button type="button" onClick={() => setInboxOpen(false)} className="text-text-muted">×</button></div></div><div className="max-h-[48vh] overflow-y-auto">{mails.map((mail, index) => <button key={mail.subject} type="button" onClick={() => { setOpenedMail(index); markOfficeMailRead(index); }} className="w-full border-b px-4 py-3 text-left transition hover:bg-cyan/10" style={{ borderColor: "rgb(var(--cyan-rgb) / .14)", background: openedMail === index ? "rgb(var(--cyan-rgb) / .08)" : undefined }}><div className="flex items-center justify-between gap-3"><b className="text-[9px] text-gold">{mail.from}</b>{!journey.readOfficeMail.includes(index) && <i className="h-2 w-2 rounded-full bg-neon-red" />}</div><div className="mt-1 text-[10px] font-black text-white">{mail.subject}</div>{openedMail === index && <p className="mt-2 text-[10px] leading-relaxed text-text-muted">{mail.preview}</p>}</button>)}</div></section>}
