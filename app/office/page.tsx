...
export default function PoliticalOfficePage() {
  ...
  const lang = useLang();
  const { leader, journey, day, totalDays, advanceDay, runLocationActivity } = useGameStore();
  ...
  return <div className="min-h-screen overflow-hidden" style={{ background: "#020814" }}>
    ...
    <main className="relative h-[calc(100vh-30px)] min-h-[650px] pt-[40px]" style={{ fontFamily: "'Space Mono', monospace" }}>
      ...
      {assistantOpen && <section className="absolute bottom-12 left-4 z-20 flex w-[min(500px,calc(100%-32px))] items-end gap-3" ><button type="button" onClick={() => setAssistantOpen(false)} className="relative h-28 w-28 shrink-0 overflow-hidden rounded-full border-2 shadow-2xl" style={{ borderColor: "var(--gold)", boxShadow: "0 0 24px rgb(var(--gold-rgb) / .42)" }}><Image src="/personal-assistant.png" alt="Personal Assistant" fill sizes="112px" className="object-cover" /></button><div className="flex-1 border p-3 shadow-2xl" style={{ borderColor: "rgb(var(--gold-rgb) / .54)", background: "rgb(2 8 20 / .94)", backdropFilter: "blur(14px)" }}><div className="flex items-start justify-between gap-3"><div><div className="text-[9px] font-black tracking-[.2em] text-gold">PEMBANTU PERIBADI</div><h2 className="mt-1 text-[13px] font-black text-white">{t(lang, "Tuan, isu penduduk makin panas.", "The resident issue is heating up.")}</h2></div><button type="button" onClick={() => setAssistantOpen(false)} className="text-text-muted">×</button></div><p className="mt-2 text-[10px] leading-relaxed text-text-muted">{t(lang, "Pilih tindakan, saya akan kemas kini terus dalam pejabat ini.", "Choose an action and I will update it directly inside this office.")}</p><div className="mt-3 flex gap-2"><button type="button" onClick={() => setActive(hotspots[1].title)} className="flex-1 border px-2 py-2 text-[9px] font-black text-cyan" style={{ borderColor: "rgb(var(--cyan-rgb) / .55)" }}>{t(lang, "ATUR LAWATAN", "SCHEDULE VISIT")}</button><button type="button" onClick={() => setActive(hotspots[2].title)} className="flex-1 border px-2 py-2 text-[9px] font-black text-cyan" style={{ borderColor: "rgb(var(--cyan-rgb) / .55)" }}>{t(lang, "KENYATAAN MEDIA", "MEDIA STATEMENT")}</button></div></div></section>}
