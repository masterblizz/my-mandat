...
export default function CityOnboarding() {
  ...
  return <section className="fixed bottom-5 left-5 right-5 z-[70] max-w-[760px] border p-4 shadow-2xl md:left-8" style={{ borderColor: "rgb(var(--cyan-rgb) / .5)", background: "rgb(var(--bg-rgb) / .92)", backdropFilter: "blur(12px)" }}>
    <div className="flex gap-3"><div className="relative hidden h-16 w-16 shrink-0 overflow-hidden border sm:block" style={{ borderColor: "rgb(var(--gold-rgb) / .55)" }}><Image src="/personal-assistant.png" alt="Personal Assistant" fill sizes="64px" className="object-cover" priority /></div><div><div className="text-[9px] font-black tracking-[.22em] text-gold">{labels[step][0]} · {lang === "ms" ? "PERSONAL ASSISTANT" : "PERSONAL ASSISTANT"}</div><h2 className="mt-1 text-sm font-black text-white">{labels[step][1]}</h2><p className="mt-1 text-[11px] text-text-muted">{labels[step][2]}</p></div></div>
